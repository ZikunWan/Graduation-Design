...
def _clientproto_ddp_worker(rank, world_size, worker_config):
    ...
    try:
        client = ClientFedProto(args, client_name=worker_config["client_name"], device=f"cuda:{device_id}")
...
class ClientFedProto(Client):
    ...
    def _train_with_brats_ddp(self, round_idx, client_idx):
        ...
        result_path = _create_ddp_result_path(f"{self.client_name.lower()}_proto_ddp")
        worker_config = {
